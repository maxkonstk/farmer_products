...
Витринные изображения проекта собраны из реальных фотографий Foodiesfeed и сохранены локально, чтобы сайт не зависел от внешнего CDN во время работы.

## Подход

- единый источник: Foodiesfeed `legacy` photo collection, без Wikimedia;
- единое кадрирование: горизонтальный формат `1200x900`;
- единая обработка: теплый тон, немного сниженная насыщенность и мягкий контраст;
- единый формат выдачи: оптимизированные локальные `jpg` в `public/images`;
- лицензия источника: Foodiesfeed License, аналог CC0, разрешает бесплатное использование и модификацию в личных и коммерческих проектах без обязательной атрибуции.

## Воспроизведение

Фотографии можно пересобрать из исходников командой:

```bash
php scripts/normalize-storefront-photos.php
```

Скрипт загружает исходные фото в локальный кеш `.cache/storefront-photo-sources` и пересоздает ассеты в каталогах:
...
Страница лицензии: https://www.foodiesfeed.com/license

| Локальный файл | Исходное фото |
| --- | --- |
